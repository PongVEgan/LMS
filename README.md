...
ทำได้ปลอดภัย ไม่มีอะไรพัง — `docker-compose.yml` ตรึงชื่อ project ไว้ด้วย `name:` แล้ว
(ถ้าไม่ตรึง Docker จะตั้งชื่อ volume ตามชื่อโฟลเดอร์ พอเปลี่ยนชื่อจะได้ฐานข้อมูลเปล่าใหม่ ข้อมูลเดิมหายหมด)

หลังเปลี่ยนชื่อ/ย้ายโฟลเดอร์:

```bash
npm run dev:kill     # ปิดเซิร์ฟเวอร์เดิมที่ยังชี้ path เก่า
rmdir /s /q .next    # ลบ build cache เก่า (Windows) — ไม่ลบก็ได้แต่ลบแล้วชัวร์กว่า
npm run demo
```

`node_modules` ไม่ต้องติดตั้งใหม่ · git ยังใช้ได้ปกติ · ไม่มีไฟล์ config ไหนอ้าง path เต็ม
