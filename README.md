...
| คำสั่ง | ทำอะไร |
|---|---|
| `npm run dev` | web + api พร้อมกัน — **โหมดพัฒนา** แก้โค้ดเห็นผลทันที แต่เปิดหน้าครั้งแรกช้า 20-60 วิ |
| `npm run demo` | web + api พร้อมกัน — **โหมดสาธิต** ทุกหน้าเปิดใน 0.2 วิ (build ก่อนรัน) |
...
| `npm run db:ui` | เปิดหน้าเว็บดูฐานข้อมูล (Adminer) ที่ http://localhost:8080 |
...
### เว็บช้าตอนเปิดหน้าครั้งแรก?

เป็นพฤติกรรมปกติของโหมดพัฒนา — Next.js compile ทีละหน้าตอนเข้าครั้งแรก (20-60 วินาที)
พอ compile แล้วจะเหลือ 0.2-0.3 วินาที

**ตอนสาธิตงานให้ใช้ `npm run demo` แทน** — build ล่วงหน้าทีเดียว แล้วทุกหน้าเปิดเร็วตั้งแต่ครั้งแรก

| หน้า | `npm run dev` (ครั้งแรก) | `npm run demo` |
|---|---|---|
| หน้าแรก | 20-28 วิ | 0.04 วิ |
| สมัครสมาชิก | 65 วิ | 0.02 วิ |

ข้อแลกเปลี่ยน: โหมดสาธิตแก้โค้ดแล้วไม่เห็นผลทันที ต้องรัน `npm run demo` ใหม่
...
## เปลี่ยนชื่อหรือย้ายโฟลเดอร์โปรเจกต์

ทำได้ปลอดภัย ไม่มีอะไรพัง — `docker-compose.yml` ตรึงชื่อ project ไว้เป็น `projectlms` แล้ว
จึงไม่ผูกกับชื่อโฟลเดอร์อีกต่อไป

> ถ้าไม่ตรึงไว้ Docker จะตั้งชื่อ volume ตามชื่อโฟลเดอร์ พอเปลี่ยนชื่อเมื่อไหร่
> จะได้ฐานข้อมูลเปล่าใหม่ทันที และข้อมูลเดิมค้างอยู่ใน volume เก่าที่ไม่มีอะไรเรียกใช้
>
> ชื่อ project ต้องเป็นตัวพิมพ์เล็กตามข้อบังคับของ Docker แต่**ชื่อโฟลเดอร์จะใช้ตัวใหญ่ก็ได้**
> ทั้งสองอย่างไม่เกี่ยวกัน

ขั้นตอนตอนเปลี่ยนชื่อ:

```bash
npm run dev:kill          # 1. ปิดเซิร์ฟเวอร์ที่ยังชี้ path เก่า
                          # 2. ปิด VS Code ที่เปิดโฟลเดอร์นี้อยู่
                          # 3. เปลี่ยนชื่อโฟลเดอร์ตามต้องการ
rmdir /s /q .next         # 4. ลบ build cache เก่า (ไม่ลบก็มักไม่พัง แต่ลบแล้วชัวร์)
npm run demo              # 5. รันใหม่
```

`node_modules` ไม่ต้องติดตั้งใหม่ · git ใช้ได้ปกติ · ไม่มีไฟล์ config ไหนอ้าง path เต็ม

## ดูฐานข้อมูลผ่านหน้าเว็บ

ตอนนำเสนอถ้าต้องการโชว์ตารางและข้อมูลจริง ใช้ **Adminer** ที่มาพร้อม Docker แล้ว
ไม่ต้องติดตั้ง pgAdmin หรือ DBeaver เพิ่ม:

```bash
npm run db:ui      # เปิด http://localhost:8080 ให้อัตโนมัติ
```

ข้อมูลเข้าสู่ระบบ (เติมช่องเซิร์ฟเวอร์ให้อัตโนมัติแล้ว):
...
เข้าไปแล้วกดชื่อตารางเพื่อดูโครงสร้าง กด **Select data** เพื่อดูข้อมูล
และมีเมนู **SQL command** ไว้รัน query สดได้

> Adminer ผูกกับ container ฐานข้อมูลโดยตรง เปิดได้เฉพาะในเครื่องที่รัน Docker เท่านั้น
